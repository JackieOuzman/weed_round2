...
### remove the crop type so we sum the outputs for all crops  (the 2016 model does not have this) ----

weed1_2_outputs_AEZ <- weed1_2_outputs_AEZ_SubCrop %>% 
...
tonnes_AEZ <- kyntec_production_data_tonnes %>% 
  group_by(AgroEcological.Zone) %>% 
  summarise(sum_production_tonnes = sum(`mean_19_21`, na.rm=TRUE))

tonnes_AEZ <- tonnes_AEZ %>%  rename(`AEZ for report` = AgroEcological.Zone)

#add to the weed1_2_outputs_AEZ df
str(weed1_2_outputs_AEZ)
str(tonnes_AEZ)

weed1_2_outputs_AEZ <- left_join(weed1_2_outputs_AEZ, tonnes_AEZ)
...
weed1_2_outputs_AEZ <- weed1_2_outputs_AEZ %>%
  mutate(
...
write.csv(weed1_2_outputs_AEZ, paste0(output_path, "fallow_weed1_2_outputs_AEZ_summary.csv"))
