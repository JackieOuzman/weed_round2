...
###############################################################################
#SELECTIVE
SELECTIVE_count_of_test_results_AEZ_SPEC <- HR_weed %>% 
  group_by(AEZ, Species, All_Species_selective) %>% 
  summarise(count_HR_TEST= n())
SELECTIVE_count_of_test_results_AEZ_SPEC


SELECTIVEcount_of_seeds_Tested_AEZ_SPEC <- HR_weed %>% 
  filter(All_Species_selective != "NOT_TESTED") %>% 
  group_by(AEZ, Species ) %>% 
  summarise(count_Seeds_TEST= n())
SELECTIVEcount_of_seeds_Tested_AEZ_SPEC

SELECTIVE <- left_join(SELECTIVE_count_of_test_results_AEZ_SPEC, SELECTIVEcount_of_seeds_Tested_AEZ_SPEC)

###############################################################################
#NON_SELECTIVE
NON_SELECTIVE_count_of_test_results_AEZ_SPEC <- HR_weed %>% 
  group_by(AEZ, Species, All_Species_non_selective) %>% 
  summarise(count_HR_TEST= n())
NON_SELECTIVE_count_of_test_results_AEZ_SPEC


NON_SELECTIVEcount_of_seeds_Tested_AEZ_SPEC <- HR_weed %>% 
  filter(All_Species_non_selective != "NOT_TESTED") %>% 
  group_by(AEZ, Species ) %>% 
  summarise(count_Seeds_TEST= n())
NON_SELECTIVEcount_of_seeds_Tested_AEZ_SPEC

NON_SELECTIVE <- left_join(NON_SELECTIVE_count_of_test_results_AEZ_SPEC, NON_SELECTIVEcount_of_seeds_Tested_AEZ_SPEC)
  

###############################################################################
#BOTH
BOTH_count_of_test_results_AEZ_SPEC <- HR_weed %>% 
  group_by(AEZ, Species, BOTH) %>% 
  summarise(count_HR_TEST= n())
BOTH_count_of_test_results_AEZ_SPEC


BOTHcount_of_seeds_Tested_AEZ_SPEC <- HR_weed %>% 
  filter(BOTH != "NOT_TESTED") %>% 
  group_by(AEZ, Species ) %>% 
  summarise(count_Seeds_TEST= n())
BOTHcount_of_seeds_Tested_AEZ_SPEC

BOTH <- left_join(BOTH_count_of_test_results_AEZ_SPEC, BOTHcount_of_seeds_Tested_AEZ_SPEC)


################################################################################

rm(BOTHcount_of_seeds_Tested_AEZ_SPEC, BOTH_count_of_test_results_AEZ_SPEC,
   NON_SELECTIVE_count_of_test_results_AEZ_SPEC, NON_SELECTIVEcount_of_seeds_Tested_AEZ_SPEC,
   SELECTIVE_count_of_test_results_AEZ_SPEC, SELECTIVEcount_of_seeds_Tested_AEZ_SPEC)
...
template <- expand.grid(AEZ, Weeds)
#name the clms
template <- template %>% 
  rename(AEZ =Var1,
         Species =Var2)
...
ALL_WEEDS <- ALL_WEEDS %>% 
  select(AEZ, Species, percenatge_weed_seed_HR) %>% 
  rename(perc_seed_HR_NON_SELECTIVE = percenatge_weed_seed_HR)

ALL_WEEDS <- left_join(ALL_WEEDS, SELECTIVE)

ALL_WEEDS <- ALL_WEEDS %>% 
  select(AEZ, Species, perc_seed_HR_NON_SELECTIVE,percenatge_weed_seed_HR) %>% 
...
write.csv(ALL_WEEDS, "W:/Economic impact of weeds round 2/HR/Jackie_working/HR_weeds/HR_Weed_HR_Class_NO_Crop_type_weed_prec.csv")
